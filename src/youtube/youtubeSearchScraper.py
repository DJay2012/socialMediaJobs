...
from apiKeysManager import apiKeysManager
...
class YouTubeSearchScraper(BaseSocialMediaScraper):
    ...
    def getApiKey(self) -> str:
        ...
        apiKeysManager.checkAndReactivateKeys()
        
        # Get key using round-robin strategy
        apiKey = apiKeysManager.getApiKey('youtube', 'round_robin')
    ...
    def fetchYoutubeData(self, query: str, maxResults: int, 
                          pageToken: Optional[str] = None) -> Optional[Dict]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    self.logger.debug(f"Successfully fetched data with {api_key[:20]}...")
                    ...
                    if 'quotaExceeded' in error_reason:
                        self.logger.warning(f"Quota exceeded for key {api_key[:20]}..., switching to next key")
                        api_keys_manager.report_error('youtube', api_key, 'quota_exceeded')
                    elif 'keyInvalid' in error_reason:
                        self.logger.warning(f"Invalid key {api_key[:20]}..., switching to next key")
                        api_keys_manager.report_error('youtube', api_key, 'invalid_key')
                    else:
                        self.logger.warning(f"API error with key {api_key[:20]}...: {error_reason}")
                        api_keys_manager.report_error('youtube', api_key, 'rate_limit')
                ...
                self.logger.error(f"Error fetching YouTube data (attempt {attempt + 1}): {e}")
    ...
    def fetch_video_details(self, video_ids: List[str]) -> Optional[Dict]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.logger.error(f"Error fetching video details (attempt {attempt + 1}): {e}")
                if attempt == max_retries - 1:
                    return None
                continue
    ...
    def fetch_channel_details(self, channel_ids: List[str]) -> Optional[Dict]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.logger.error(f"Error fetching channel details (attempt {attempt + 1}): {e}")
                if attempt == max_retries - 1:
                    return None
                continue
    ...
    def processSingleKeyword(self, keywordData: Dict[str, str]) -> bool:
        ...
        try:
            ...
            while True:
                ...
                videoDetails = self.retryWithBackoff(
                    self.fetchVideoDetails, videoIds
                )
                ...
                successCount = self.processVideos(
                    searchData, videoDetails, channelDetails, 
                    keywordData, collection
                )
        ...
        for item in search_data.get('items', []):
            if item['id']['kind'] == 'youtube#video':
                try:
                    ...
                    video_data['profile_image'] = channel_info.get(
                        channel_id, {}
                    ).get('profile_image')
                    video_data['location'] = channel_info.get(
                        channel_id, {}
                    ).get('location')
