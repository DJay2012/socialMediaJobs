...
def fetch_and_save_tweets(query_params, max_results, next_token=None):
    try:
        ...
        if tweets_data:
            ...
            media = {media['media_key']: media.get('url', '') for media in tweets_data.get('includes', {}).get('media', [])}
            ...
            for tweet in tweets:
                ...
                tweet['created_at'] = datetime.strptime(created_at, "%Y-%m-%dT%H:%M:%S.%fZ").replace(tzinfo=tz) if created_at else ''
                
                if 'attachments' in tweet and 'media_keys' in tweet['attachments']:
                    tweet['media_urls'] = [media.get(media_key, '') for media_key in tweet['attachments']['media_keys']]
                else:
                    tweet['media_urls'] = []
        ...
        try:
            ...
            if existing_tweet:
                ...
                tag_exists = any(
                    ...
                    tag["companyId"] == tweet['tags'][0]["companyId"] and
                    tag["companyName"] == tweet['tags'][0]["companyName"] and
                    tag["clientName"] == tweet['tags'][0]["clientName"]
