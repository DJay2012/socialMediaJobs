...
class BaseSocialMediaScraper(ABC):
    ...
    def connectToDatabase(self):
        ...
        try:
            ...
            self.logger.info(f"Connected to MongoDB for {self.platform_name}")
    ...
    def parsePublishedAt(self, publishedAt: str) -> datetime:
        ...
        tz = pytz.UTC
        try:
            return datetime.strptime(publishedAt, "%Y-%m-%dT%H:%M:%S.%fZ").replace(tzinfo=tz)
        except ValueError:
            return datetime.strptime(publishedAt, "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=tz)
    
    def addClientTags(self, data: Dict[str, Any], clientInfo: Dict[str, str]) -> Dict[str, Any]:
    ...
    def checkAndUpdateExistingRecord(self, collection, recordId: str, newData: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if existingRecord:
                ...
                tagExists = any(
                    tag.get("clientId") == newData['tags'][0]["clientId"] and
                    tag.get("companyId") == newData['tags'][0]["companyId"]
                ...
                if 'followers_info' in newData and 'followers_info' in existingRecord:
                    if existingRecord['followers_info'] != newData['followers_info']:
                        updateFields["$set"] = {"followers_info": newData['followers_info']}
                ...
                if updateFields:
                    ...
                    self.logger.debug(f"No updates needed for {self.platform_name} record: {recordId}")
                    return False
            else:
                # Insert new record
                collection.insert_one(newData)
                self.logger.info(f"Inserted new {self.platform_name} record: {recordId}")
                return True
                
        except DuplicateKeyError:
            self.logger.warning(f"Duplicate key error for {self.platform_name} record: {recordId}")
            return False
        except Exception as e:
            self.logger.error(f"Error processing {self.platform_name} record {recordId}: {e}")
            return False
    ...
    def rateLimitDelay(self):
        ...
        delay = random.uniform(config.app.rate_limit_delay, config.app.rate_limit_delay * 2)
        time.sleep(delay)
    
    def getSearchKeywords(self, searchType: str, clientFilter: Optional[str] = None) -> List[Dict[str, str]]:
    ...
    def run(self, searchType: str, clientFilter: Optional[str] = None):
        ...
        try:
            ...
            self.logger.info(f"Processing {len(keywords)} keywords for {self.platform_name}")
            ...
            for keywordData in keywords:
                try:
                    ...
                    if success:
                        self.logger.info(f"Successfully processed: {keywordData['query']}")
                    else:
                        self.logger.warning(f"Failed to process: {keywordData['query']}")
                    
                    # Rate limiting delay
                    self.rateLimitDelay()
                    
                except Exception as e:
                    self.logger.error(f"Error processing keyword {keywordData['query']}: {e}")
                    continue
