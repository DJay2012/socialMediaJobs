...
def is_run_finished(run_id):
    response = requests.get(f"{BASE_URL}/actor-runs/{run_id}?token={APIFY_API_TOKEN}")
...
def get_run_data(run_id, query, clientId, clientName, companyId, companyName):
    ...
    if response.status_code == 200:
        ...
        for post in data:
            ...
            post["author"] = post.get("author", {"id": None, "name": None, "url": None})
            ...
            if created_at:
                tz = pytz.UTC
                post["createdAt"] = datetime.strptime(
            ...
            try:
                ...
                if existing_post:
                    ...
                    tag_exists = any(
                        ...
                        and tag["companyName"] == companyName
                        and tag["clientName"] == clientName
                    ...
                    if not tag_exists:
                        facebook_collection.update_one(
                            {"_id": post["_id"]}, {"$push": {"tags": post["tags"][0]}}
                        )
