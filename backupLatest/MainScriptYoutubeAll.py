...
# API_KEY = 'AIzaSyDQdFCMR3dhfj8rBiVnQO0mkwe6Tta8xZ8'
# API_KEY = "AIzaSyBCmfIHRbtzZTeUHcBnjhcRCm5LHogSDhk"
...
# Function to fetch search terms from MongoDB
def get_search_terms():
    documents = search_keywords_collection.find({"type": "youtube"})
    search_terms_data = []
...
def main():
    ...
    for search_term in search_terms_data:
        ...
        while True:
            ...
            video_ids = [item['id']['videoId'] for item in search_data.get('items', []) if item['id']['kind'] == 'youtube#video']
            ...
            for item in search_data.get('items', []):
                if item['id']['kind'] == 'youtube#video':
                    ...
                    video_info = next((video for video in video_details.get('items', []) if video['id'] == video_id), {})
                    
                    item['statistics'] = video_info.get('statistics', {})
                    channel_id = item['snippet']['channelId']
                    item['profile_image'] = channel_info.get(channel_id, {}).get('profile_image')
                    item['location'] = channel_info.get(channel_id, {}).get('location')
                    item['video_link'] = f'https://www.youtube.com/watch?v={video_id}'
                    
                    item['_id'] = video_id
                    item['keywords'] = query
                    item['createdAt'] = parse_published_at(item['snippet']['publishedAt'])
                    ...
                    mongo_collection.update_one({'_id': video_id}, {'$set': item}, upsert=True)
