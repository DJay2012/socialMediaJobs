...
class SocialFeedSchema(BaseModel):
    ...
    def from_youtube(
        ...
        # Deduplicate company tags by companyId (in case same companyId appears with different clientIds)
        seen_company_ids = set()
        unique_company_tags = []
        for company_tag in company_tags:
            if company_tag.id and company_tag.id not in seen_company_ids:
                seen_company_ids.add(company_tag.id)
                unique_company_tags.append(company_tag)

        processed_doc["companyTag"] = unique_company_tags

        # Create socialFeedTag entries - one per unique companyId
        social_feed_tags = []
        for company_tag in unique_company_tags:
