...
class YouTubeScraper(BaseScraper):
    ...
    def _get_tags(self, data: Dict[str, Any]):
        """
        Get tags from data, supporting both single and multiple clientIds/companyIds.
        Supports backward compatibility with single clientId/companyId fields,
        and new format with clientIds/companyIds arrays.
        """
        tags = []
        
        # Support new format: arrays of clientIds and companyIds
        client_ids = data.get("clientIds", [])
        company_ids = data.get("companyIds", [])
        client_names = data.get("clientNames", [])
        company_names = data.get("companyNames", [])
        
        # Support backward compatibility: single clientId/companyId
        if not client_ids and data.get("clientId"):
            client_ids = [data.get("clientId", "")]
        if not company_ids and data.get("companyId"):
            company_ids = [data.get("companyId", "")]
        if not client_names and data.get("clientName"):
            client_names = [data.get("clientName", "")]
        if not company_names and data.get("companyName"):
            company_names = [data.get("companyName", "")]
        
        # Generate all combinations of clientIds and companyIds
        for client_idx, client_id in enumerate(client_ids):
            client_name = client_names[client_idx] if client_idx < len(client_names) else ""
            
            for company_idx, company_id in enumerate(company_ids):
                company_name = company_names[company_idx] if company_idx < len(company_names) else ""
