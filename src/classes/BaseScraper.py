...
class BaseScraper(ABC):
    ...
    def check_and_update_existing_record(
        ...
        """Check if record exists and update if needed, merging tags instead of replacing"""
        try:
            existingRecord = collection.find_one({"_id": recordId})

            if existingRecord:
                # Get new tags (support both single tag and multiple tags)
                new_tags = newData.get("tags", [])
                if not isinstance(new_tags, list):
                    new_tags = [new_tags] if new_tags else []
                
                existing_tags = existingRecord.get("tags", [])
                
                # Create a set of existing tag identifiers for quick lookup
                existing_tag_keys = {
                    (tag.get("clientId", ""), tag.get("companyId", ""))
                ...
                tags_to_add = [
                    ...
                    if (tag.get("clientId", ""), tag.get("companyId", "")) not in existing_tag_keys
                ]
                ...
                # Add new tags if any don't exist
                if tags_to_add:
                    if "$push" not in updateFields:
                        updateFields["$push"] = {}
                    updateFields["$push"]["tags"] = {"$each": tags_to_add}

                # Update other fields (excluding tags and _id)
                for key, value in newData.items():
                    if key not in ["tags", "_id", "followers_info", "statistics"]:
                        if "$set" not in updateFields:
                            updateFields["$set"] = {}
    ...
    def bulk_insert_or_replace(
        ...
        If _id exists, merge tags instead of replacing; otherwise insert new document.
                ...
                if existing_doc:
                    ...
                    if tags_to_add:
                        if "$push" not in update_fields:
                            update_fields["$push"] = {}
                        update_fields["$push"]["tags"] = {"$each": tags_to_add}
                    
                    # Only perform update if there are changes
                    if update_fields:
                        operations.append(UpdateOne({"_id": record_id}, update_fields, upsert=False))
                    else:
                        # No changes needed, count as replaced but skip operation
                        results["replaced"] += 1
                else:
