...
def search_influencer_in_channel(
    channelName, influencerName, clientId, clientName, companyId, CompanyName
):
    ...
    if channel_id:
        ...
        # influencerName --> keywods[BMw,Mini,X5,X7,Z4]
        filtered_videos = [
            video
            for video in videos
            if influencerName.lower() in video["snippet"]["title"].lower()
            or influencerName.lower() in video["snippet"]["description"].lower()
        ]
        ...
        if filtered_videos:
            for video in filtered_videos:
                ...
                if existing_document:
                    ...
                    if (
                        existing_stats.get("view_count") != stats["viewCount"]
                        or existing_stats.get("like_count")
                        != stats.get("likeCount", "Not available")
                        or existing_stats.get("comment_count")
                        != stats.get("commentCount", "Not available")
                    ):
...
def process_keywords():
    ...
    for keyword in keywords:
        channelName = keyword["channelName"]
        influencerName = keyword["influencerName"]
