...
def list_keys(service: str = None):
    ...
    if service:
        ...
        for service_name in api_keys_manager.keys:
            ...
            print(f"{service_name.upper()}: {status['active_keys']}/{status['total_keys']} active")
...
def main():
    """Main CLI function"""
    parser = argparse.ArgumentParser(description='Manage API keys for social media scrapers')
    subparsers = parser.add_subparsers(dest='command', help='Available commands')
    
    # List command
    list_parser = subparsers.add_parser('list', help='List API keys')
    list_parser.add_argument('--service', help='Filter by service (youtube, twitter, apify)')
